...
import { startScheduler } from '../../src/server/services/checkinScheduler.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    res.status(200).json({ success: true, message: 'Checkin task executed' });
  } catch (error) {
    console.error('Error executing checkin task:', error);
